...
DEFAULT_STATUS_PATH = DEFAULT_RESEARCH_ROOT / "sf3-development-latest.json"
DEFAULT_PROTOCOL_PATH = Path("config/sf3_research_protocol_v1.json")
...
def _load_reusable_status(
    *,
    ...
    if not status_path.is_file():
        return None
    try:
        payload = _read_object(status_path, label="SF3 runtime status")
    except RuntimeError:
        return None
    checks = (
    ...
    if not development_path.is_file() or not validation_path.is_file():
        return None
    try:
        development = _read_object(development_path, label="SF3 development report")
        validation = _read_object(validation_path, label="SF3 validation report")
    except RuntimeError:
        return None
    report_checks = (
    ...
    *,
    ...
    try:
        ...
        if materialization.corpus_id != protocol.corpus.corpus_id:
            raise RuntimeError("SF3 materialization corpus identity mismatch")
        if len(materialization.windows) != len(protocol.corpus.windows):
            raise RuntimeError("SF3 materialization window count mismatch")
        if any(
            not isinstance(window.feature_csv_sha256, str)
            or len(window.feature_csv_sha256) != 64
            for window in materialization.windows
        ):
            raise RuntimeError("SF3 materialization lacks feature integrity hashes")
        ...
        development = evaluate_sf3_development(
        ...
        if not all(development_checks):
            raise RuntimeError("SF3 development report identity or safety mismatch")
        evaluation_id = str(development.get("evaluationId") or "")
        if not evaluation_id:
            raise RuntimeError("SF3 development report is missing evaluationId")
        development_path = evidence_root / f"sf3-development-{evaluation_id}.json"
        write_immutable_report(development_path, development)

        validation = validate_sf3_development(development, protocol_path=protocol_path)
        validation_checks = (
            _safe_contract(validation, schema=VALIDATION_REPORT_SCHEMA),
        ...
        if not all(validation_checks):
            raise RuntimeError("SF3 validation report identity or safety mismatch")
        validation_id = str(validation.get("validationId") or "")
        if not validation_id:
            raise RuntimeError("SF3 validation report is missing validationId")
        validation_path = evidence_root / f"sf3-validation-{validation_id}.json"
