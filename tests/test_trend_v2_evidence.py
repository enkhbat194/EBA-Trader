...
from eba_trader.trend_v2 import TrendV2Result
from eba_trader.trend_v2_evidence import (
    FoldResult,
    NeighborhoodResult,
    blocked_risk_gates,
    evaluate_risk_gates,
...
def _result(**changes) -> TrendV2Result:
    values = {
    ...
    return TrendV2Result(**values)


def test_all_23_signal_gates_pass_on_exactly_qualifying_fixture() -> None:
    validation = _result()
    ...
    assert len(gates) == 23
    assert [gate.number for gate in gates] == list(range(1, 24))
    assert all(gate.status == "PASS" for gate in gates)
    assert gates[17].name == "baseline_anti_peak"


def test_zero_trade_folds_fail_predeclared_positive_metric_gates() -> None:
    validation = _result()
    severe = _result()
    control = _result(
        expectancy=0.5,
...
def test_risk_gates_and_blocked_gate_numbering_are_complete() -> None:
    ...
    assert [gate.number for gate in risk_gates] == list(range(24, 37))
    assert all(gate.status == "PASS" for gate in risk_gates)
    assert [gate.number for gate in blocked] == list(range(24, 37))
